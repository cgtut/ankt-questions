document.addEventListener('DOMContentLoaded', () => {
    ...
    let allTestData = [];
    let testData = [];
    let QUESTIONS_URL = 'questions.json';
    ...
    const loadAltJsonBtn = document.getElementById('loadAltJsonBtn');

    loadAltJsonBtn.addEventListener('click', () => {
  // пример: второй файл в том же репозитории questions2.json
  loadQuestions('questions2.json');

  // если хотите сбросить текущий тест:
  currentQuestionIndex = 0;
  userAnswers = {};
  correctAnswersCount = 0;
  testContainer.innerHTML = 'Новый набор вопросов загружен. Нажмите "Начать анкету".';
